...
class MedNeXtBottleneckBlock(nn.Module):
    def __init__(self, in_channels, kernel_size=5, expansion=4, groups=32, downsample=True, channel_multiplier=2):
        super(MedNeXtBottleneckBlock, self).__init__()
        self.kernel_size = kernel_size
        self.in_channels = in_channels
        self.expansion = expansion
        self.downsample = downsample
        self.out_channels = in_channels * channel_multiplier  # 🔹 Increase output channels

        # Depthwise convolution (Reduces spatial dim if downsample=True)
        self.depthwise_conv = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, 
                                        stride=2 if downsample else 1, groups=in_channels, 
                                        padding=kernel_size // 2, bias=False)

        # Group Normalization
        self.gn = nn.GroupNorm(num_groups=groups, num_channels=in_channels)

        # Pointwise convolution (Expanding channels)
        self.pointwise_conv1 = nn.Conv2d(in_channels, self.out_channels, kernel_size=1, bias=False)

        # GELU activation
        self.gelu = nn.GELU()

        # Pointwise convolution (Reducing back to final output size)
        self.pointwise_conv2 = nn.Conv2d(self.out_channels, self.out_channels, kernel_size=1, bias=False)

        # 🔹 Skip Connection Downsampling & Channel Matching
        if downsample or in_channels != self.out_channels:
            self.residual_downsample = nn.Conv2d(in_channels, self.out_channels, kernel_size=1, stride=2 if downsample else 1, bias=False)
        else:
            self.residual_downsample = nn.Identity()

    def forward(self, x):
        residual = self.residual_downsample(x)  # 🔹 Adjust residual shape

        # Depthwise convolution (with optional spatial downsampling)
        ...
        # Second Pointwise convolution (compression)
