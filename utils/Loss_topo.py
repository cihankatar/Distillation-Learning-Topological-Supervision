...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_outputs, teacher_outputs, teacher_temp):
        ...
        for t_out in teacher_probs:  # loop over teacher views (2)
            for s_out in student_logits:  # loop over student views (2)
                # Compute topo_loss per image
                topo_loss = self.topo_loss(t_out,s_out)
...
class Topological_Loss(torch.nn.Module):
    ...
    def forward(self, model_output,labels):
        ...
        totalloss             = 0
        model_output          = self.sigmoid_f(model_output)
        predictions           = torch.squeeze(model_output,dim=1) 
