...
class DINOLoss(nn.Module):
    ...
    def forward(
        ...
        for teacher_index, t_out in enumerate(teacher_probs):
            for student_index, s_log in enumerate(student_logits):
                # The first student views are the same spatial global views as
                # the teacher views.  Standard DINO excludes identical-view
                # pairs and learns from cross-view predictions instead.
                if exclude_matching_views and teacher_index == student_index:
                    continue
                loss = torch.sum(-t_out * F.log_softmax(s_log, dim=-1), dim=-1).mean()
                total_loss += loss
                n_loss_terms += 1
        ...
        if n_loss_terms == 0:
            raise ValueError("DINOLoss received no valid teacher/student view pairs")
